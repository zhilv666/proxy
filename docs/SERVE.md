...
`proxy serve` 启动内嵌网页，用于管理当前代理配置、已保存节点和命令别名。服务和网页都包含在同一个可执行文件中。
...
| 操作 | 结果 |
| --- | --- |
| 保存当前代理 | 更新协议、主机、端口、用户名、密码；有关联节点时同步写回该节点 |
| 恢复默认 | 当前代理恢复为 `http://127.0.0.1:7890`，清空认证并解除关联，保留已保存节点和别名 |
| 添加节点 / 另存为节点 | 新建一份独立配置，当前连接保持原样 |
| 启用节点 | 整体替换当前代理配置，后续 CLI 命令使用该节点 |
| 编辑或重命名激活节点 | 同步更新当前代理参数和关联名称 |
| 解除节点关联 | 保留当前代理参数，后续修改当前配置不再写回原节点 |
| 删除激活节点 | 删除保存的节点，保留当前代理参数并解除关联 |
| 编辑别名 | 可同时修改名称、命令和平台；与其他别名冲突时拒绝保存 |
