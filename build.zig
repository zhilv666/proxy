...
pub fn build(b: *std.Build) void {
    ...
    // Unit tests (main.zig 聚合 check/config 的测试)
    ...
    unit_tests.root_module.addOptions("build_info", build_opts);
...
fn buildTimestamp(b: *std.Build) []const u8 {
    // 默认按东八区 (UTC+8) 显示构建时间;本地已是 UTC+8 时直接等效于系统时间。
    const secs: u64 = @intCast(std.time.timestamp() + 8 * 3600);
    const es = std.time.epoch.EpochSeconds{ .secs = secs };
    const yd = es.getEpochDay().calculateYearDay();
    const md = yd.calculateMonthDay();
    const ds = es.getDaySeconds();
    return b.fmt("{d:0>4}-{d:0>2}-{d:0>2} {d:0>2}:{d:0>2} +08", .{
        yd.year,
        md.month.numeric(),
        md.day_index + 1,
        ds.getHoursIntoDay(),
